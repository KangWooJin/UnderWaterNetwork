...
/**
 * \class UanHeaderCommon1
 *
 * Header includes 1 byte src address, 1 byte dest address,
 * and a 1 byte type field.
 */
class UanHeaderCommon1 : public Header
{
public:
  UanHeaderCommon1 ();
  /**
   * \brief Create UanHeaderCommon1 object with given source and destination address and header type
   * \param src Source address defined in header
   * \param dest Destination address defined in header
   * \param type Header type
   */
  UanHeaderCommon1 (const UanAddress src, const UanAddress dest, uint8_t type);
  virtual ~UanHeaderCommon1 ();
